...
#include "e-ink/DEV_Config.h"
#include "e-ink/GUI_Paint.h"
#include "e-ink/EPD_1in54_V2.h"
#include "app_msgstructs.h"
#include "lora_porting.h"


#define TAG "LoRaWAN-Sensor"
#define DEVICE 										SENSOR_DEVICE
#define LORAWAN_NVM_BASE_ADDRESS                    ((void *)0x0803F000UL)
#define DEFAULT_MAX_SEND_COUNT 						(3U)

#define JOIN_PERIOD_SEC 							(5U)
#define RESEND_DELAY_SEC 							(3U)
#define UPDATE_ENVDATA_PERIOD_SEC					(30U)



#define ACTION_UPDATE_ENVDATA  						(uint8_t)APPMSG_ENVDATA_REQ
#define ACTION_PAIR_REQUEST  						(uint8_t)APPMSG_PAIR_REQ
#define ACTION_PAIR_STAT_REQUEST  					(uint8_t)APPMSG_PAIR_STAT_REQ
...
static const char *ActionString[] = {
	[APPMSG_ENVDATA_REQ] 	= "Start send environment data",
	[APPMSG_PAIR_REQ] 		= "Start pair request",
	[APPMSG_PAIR_STAT_REQ] 	= "Start pair status request",
};
static const char *ActionResponsedString[] = {
	[APPMSG_ENVDATA_RES] 	= "Receive environment data response",
	[APPMSG_PAIR_STAT_RES] 	= "Receive pair status response",
};
...
static TimerEvent_t timer_UpdateEnvData;

static UBYTE *E_Ink_RAM;
...
static void OnTimerUpdateEnvData(void*);
...
static void display_environment_data(float temp, float humi, float set, int is_paired);
...
void LoRaWAN_Init(void){
	...
	if (!shtc3_read_id(&hi2c2)) {
		LOGE(TAG, "Temp-Hum Sensor error.");
		Error_Handler();
	}
    ...
    if((E_Ink_RAM = (UBYTE *)malloc(Imagesize)) == NULL) {
    	LOGE(TAG, "E-Ink RAM allocate failed.");
		Error_Handler();
    }
    Paint_NewImage(E_Ink_RAM, EPD_1IN54_V2_WIDTH, EPD_1IN54_V2_HEIGHT, 270, WHITE);
	EPD_1IN54_V2_Init_Partial();
    Paint_SelectImage(E_Ink_RAM);
	...
	TimerInit(&timer_UpdateEnvData, OnTimerUpdateEnvData);
	TimerSetValue(&timer_UpdateEnvData, UPDATE_ENVDATA_PERIOD_SEC*1000U);
	EnableSleepMode(true);

	if (shtc3_perform_measurements(&hi2c2, &temp, &humi))
		display_environment_data((float)temp/100.0, (float)humi, srv_response.temperature_set, srv_response.is_paired);
...
static void Task_AppProcess(void *){
//	appmsg_types_t action_bootup = ACTION_PAIR_STAT_REQUEST;
...
static void OnTxData(LmHandlerTxParams_t *params){
	...
	if (is_pair_request) {
		...
		LOGI(TAG, "Waiting for pair.");
		EnterSleepModeOn(10000);
	...
	if (LmHandlerJoinStatus() == LORAMAC_HANDLER_SET){
		if (appData->BufferSize != 0){
			if (appmsg_parse_message(appData->Buffer, appData->BufferSize, &srv_response, DEVICE)){
				...
				shtc3_perform_measurements(&hi2c2, &temp, &humi);
				display_environment_data((float)temp/100.0, (float)humi, srv_response.temperature_set, srv_response.is_paired);
			}
			else
				goto action_done;
...
static void joined_startapp(void) {
	TimerStop(&timer_UpdateEnvData);
	TimerStart(&timer_UpdateEnvData);
...
static void send_uplink_message(appmsg_types_t type){
	float temp_val, humi_val, batt_level;

	if (type == APPMSG_ENVDATA_REQ) {
		if (shtc3_perform_measurements(&hi2c2, &temp, &humi)) {
			temp_val = (float)temp/100.0;
			humi_val = (float)humi;
			display_environment_data(temp_val, humi_val, srv_response.temperature_set, srv_response.is_paired);
		}
...
static void attemp_tosend_uplink(appmsg_types_t action) {
	...
	if (uplink_action_count <= DEFAULT_MAX_SEND_COUNT) {
		...
		if (status != LORAMAC_HANDLER_SUCCESS)
			LOGE(TAG, "Send uplink message failed with code %d", status);
	}
...
//	TimerSetValue(&timer_UpdateEnvData, UPDATE_ENVDATA_PERIOD_SEC*1000U);
	...
	BaseType_t yield;
	appmsg_types_t action = ACTION_UPDATE_ENVDATA;
...
//	LOGM("", "***** TEMP: %.02fC ***** :", temp);
//	LOGM("", "***** HUMI: %.02f%% ***** :", humi);
//	LOGM("", "***** PAIR: %s ****** :", is_paired ? "YES": "NO ");
//	LOGM("", "************************ :");
	Paint_Clear(WHITE);

	memset(buf, 0, sizeof(buf));
	sprintf(buf, "TSET:%.02fC", set);
	Paint_DrawString_EN(2, 10, buf, &Font24, WHITE, BLACK);
	memset(buf, 0, sizeof(buf));
	sprintf(buf, "TEMP:%.02fC", temp);
	Paint_DrawString_EN(2, 34, buf, &Font24, WHITE, BLACK);
	memset(buf, 0, sizeof(buf));
	sprintf(buf, "HUMI:%.02f%%", humi);
	Paint_DrawString_EN(2, 58, buf, &Font24, WHITE, BLACK);
	memset(buf, 0, sizeof(buf));
	sprintf(buf, "PAIR:%s", is_paired ? "YES": "NO ");
	Paint_DrawString_EN(2, 82, buf, &Font24, WHITE, BLACK);

	EPD_1IN54_V2_DisplayPart(E_Ink_RAM);
}
