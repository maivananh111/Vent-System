...
uint16_t shtc3_read_id(I2C_HandleTypeDef *hi2c) {
	...
	if (code == 0x807)
		return id & SHTC3_SENSOR_ID_MASK; // Sensor preset, return actual ID
...
static uint32_t _perform_measurements(I2C_HandleTypeDef *hi2c, uint16_t command,
		int32_t *out_temp, int32_t *out_hum) {
	uint8_t result[6];

	uint32_t res = HAL_I2C_Master_Transmit(hi2c, SHTC3_ADDRESS_WRITE,
			(uint8_t*) &command, 2, 100);
	if (res != HAL_OK) {
		return 0;
	}
...
uint32_t shtc3_perform_measurements(I2C_HandleTypeDef *hi2c, int32_t *out_temp,
		int32_t *out_hum) {
	return _perform_measurements(hi2c, SHTC3_CMD_CLK_STRETCH_READ_HUM_FIRST,
			out_temp, out_hum);
}

uint32_t shtc3_perform_measurements_low_power(I2C_HandleTypeDef *hi2c,
		int32_t *out_temp, int32_t *out_hum) {
	return _perform_measurements(hi2c,
			SHTC3_CMD_CLK_STRETCH_READ_HUM_FIRST_LOW_POWER, out_temp, out_hum);
}
